...
class AVRider(BaseController):
    ...
    def get_accel(self, env):
        ...
        if self.rl_agent: #use the rl agent to determine the acceleraiton command if the rl-agent is assigned
            lead_id = env.k.vehicle.get_leader(self.veh_id)
            lead_vel = env.k.vehicle.get_speed(lead_id)
            this_vel = env.k.vehicle.get_speed(self.veh_id)
            headway = env.k.vehicle.get_headway(self.veh_id)
            state = self.rl_get_state(lead_vel,this_vel,headway)
            AVAccelCmd = self.rl_agent.compute_action(state)
        else:  
    ...
    def rl_get_state(self,lead_v,this_v,headway):
