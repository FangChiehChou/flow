...
hv = 'IDM';
FLOW_Sim_Results_path ='/media/lorr/TOSHIBA EXT/lorr_sim_out';

Sim_AV_data=cell(length(av_case)*22,1);
AV_data_index = 0;

AV_stabilize_time = zeros(length(av_case),22,Sim_Runs);
AV_stabilize_speed = zeros(length(av_case),22,Sim_Runs);
AV_max_gap = zeros(length(av_case),22,Sim_Runs);
AV_total_fuel = zeros(length(av_case),22,Sim_Runs);
AV_VMT = zeros(length(av_case),22,Sim_Runs);
...
%Iterate through each AV type
    ...
    for av_num = 1:1:22  %iterate through experiments of different number of AVs on the ring
        ...
        for file = files'   %check all the excel file in this expereiment and compute each vehicle's speed cycle
            ...
            AV_stabilize_time(av_case_index,av_num,index_run) = stabilizing_time;  %zeros(length(av_case),22);
...
%% Save 
save('RL_results.mat','AV_stabilize_time','AV_stabilize_speed','AV_max_gap','AV_total_fuel','AV_VMT')
