...
hv = 'IDM';
FLOW_Sim_Results_path ='/home/lorr/flow/FLOR/MATLAB_code/IDM_AVRider_EvenInterval_1101/';
...
%Iterate through each AV type
    ...
    for av_num_index = 1:1:length(av_num_case)  %iterate through experiments of different number of AVs on the ring
        av_num = av_num_case(av_num_index);
        AV_data_index = AV_data_index+1;
        hv_num = 22-av_num;
        path_scenario = [FLOW_Sim_Results_path,hv,'_AVRider','_',av,'_EvenInterval'];  %%directory for simulation results

        Matlab_results_dir = [pwd,['/LOR_EvenInterval/',hv,'_',av,'/',]];   %%Making directory for saving figures from MATLAB
...
%% Saving Processed Data of this experiment.
    ...
    eval(['save(''EvenInterval_',av,'.mat'',''Sim_AV_data_',av,''');'])
...
saveas(gcf,['/home/lorr/flow/FLOR/MATLAB_code/IDM_AVRider_EvenInterval/',filename]);
close all   
...
%% Save 
save('Temp_1101.mat','AV_stabilize_time','AV_stabilize_speed','AV_max_gap')
